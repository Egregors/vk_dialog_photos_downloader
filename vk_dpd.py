...
@click.option('-d', '--dialog_id',
              help='dialog id ("sel=DIALOG_ID" from URL) or "all" for tre download all',
              required=True)
...
def download_photos(remixsid: str, dialog_id: str, username: str, debug: bool):
    ...
    :return:
    """
    def download(username: str = username, dialog_id: str = dialog_id) -> int:
        data = {
            'act': 'show',
            'al': 1,
            'loc': 'im',
        ...
        if file_count == 0:
            ...
            if os.path.exists(username):
                ...
                click.secho('Не могу удалить пустую папку', fg='rad')
    ...
    if dialog_id != 'all':
        ...
        click.secho('Начинаю перебор..', fg='green')
        g_file_count = 0
        for id in range(0, 9999999999):
            click.clear()
            click.secho(' Пробуем для id: {}'.format(str(id)), fg='green')
            g_file_count += download(dialog_id=str(id))

        click.secho('Готово! Скачено {} фото'.format(g_file_count), fg='green')
